...
import { formatDate, InsightCards, NewsCard, RemoteBoundary, StatusPanel, useRemote } from '../components'
...
export function NewsPageView() {
  ...
  const state = useRemote(() => api<NewsPage>(`/api/v1/news${queryString({ stockCode, sentiment, importance, sort, cursor, limit: 24 })}`), [stockCode, sentiment, importance, sort, cursor])
  return <div className="content-page"><PageTitle eyebrow="NEWS INTELLIGENCE" title="Signals behind the headlines" copy="Sentiment and importance remain separate signals. AI summaries never replace the original source." />
    <div className="filter-bar"><label>Stock code<input value={stockCode} maxLength={6} onChange={(event) => setStockCode(event.target.value.toUpperCase())} placeholder="005930" /></label><label>Sentiment<select value={sentiment} onChange={(event) => setSentiment(event.target.value)}><option value="">All</option><option>POSITIVE</option><option>NEUTRAL</option><option>NEGATIVE</option></select></label><label>Importance<select value={importance} onChange={(event) => setImportance(event.target.value)}><option value="">All</option><option>HIGH</option><option>MEDIUM</option><option>LOW</option></select></label><label>Sort<select value={sort} onChange={(event) => setSort(event.target.value)}><option value="LATEST">Latest</option><option value="IMPORTANCE">Importance</option><option value="MARKET_IMPACT">Market impact</option></select></label></div>
...
export function NewsDetailPage({ articleId }: { articleId: string }) {
  ...
  return <div className="content-page article-page"><RemoteBoundary state={state}>{(article) => <>
    <a className="back-link" href={appHash('news')}>← Back to News</a><header className="article-head"><div className="news-tags"><span>{article.publisher}</span><span>{formatDate(article.publishedAt)}</span><span className={`sentiment ${(article.sentiment || '').toLowerCase()}`}>{article.sentiment || 'Pending'}</span><span>{article.importance || 'Unrated'} importance</span></div><h1>{article.englishTitle || article.originalTitle}</h1><div className="stock-chips">{article.relatedStocks.map((stock) => <a key={stock.stockCode} href={appHash('stock-detail', stock.stockCode)}>{stock.nameEn || stock.nameKo} · {stock.stockCode}</a>)}</div></header>
    ...
    <div className="article-layout"><article className="article-body" ref={articleRef} onMouseUp={captureSelection}><div className="translation-label"><b>English translation</b><span>{article.analysisStatus} · paragraph structure preserved when available</span></div>{(article.englishBody || article.originalBody || article.originalExcerpt || '').split(/\n{2,}/).map((paragraph, index) => <p key={index}>{paragraph}</p>)}{!article.englishBody && <StatusPanel title="English translation unavailable" message="The original Korean text is shown. No synthetic translation was inserted." tone="warning" />}<a className="source-button" href={article.originalUrl} target="_blank" rel="noreferrer">Open original article ↗</a></article>
      <aside className="analysis-rail"><h3>Analysis signals</h3><dl><div><dt>Event</dt><dd>{article.eventType || 'Pending'}</dd></div><div><dt>Sentiment</dt><dd>{article.sentiment || 'Pending'}</dd></div><div><dt>Importance</dt><dd>{article.importance || 'Pending'}</dd></div><div><dt>Market impact</dt><dd>{article.marketImpact || 'Pending'}</dd></div></dl><p>Confidence values describe model certainty, not expected return.</p></aside></div>
    {selection && <div className="selection-popup" style={{ left: selection.x, top: selection.y }}><span>“{selection.text.slice(0, 48)}{selection.text.length > 48 ? '…' : ''}”</span><button onClick={() => void explain()} disabled={explaining}>✦ {explaining ? 'Explaining…' : 'Explain this term'}</button></div>}
...
export function DartPage() {
  ...
  const state = useRemote(() => api<FilingPage>(`/api/v1/disclosures${queryString({ stockCode, types: type, from: dates.from, to: dates.to, cursor, limit: 30 })}`), [stockCode, type, dates.from, dates.to, cursor])
  return <div className="content-page"><PageTitle eyebrow="DART INTELLIGENCE" title="Corporate filings, grounded in source" copy="OpenDART filings for the 75-stock universe. Each filing is isolated by receipt number and document version." />
    ...
    <div className="filter-bar"><label>Stock code<input value={stockCode} onChange={(event) => setStockCode(event.target.value.toUpperCase())} maxLength={6} placeholder="All stocks" /></label><label>Date range<select value={range} onChange={(event) => setRange(event.target.value)}><option>1M</option><option>3M</option><option>6M</option><option>1Y</option></select></label><label>Filing type<select value={type} onChange={(event) => setType(event.target.value)}><option value="">All types</option><option>PERIODIC</option><option>MATERIAL_EVENT</option><option>ISSUANCE</option><option>OWNERSHIP</option><option>OTHER</option></select></label></div>
