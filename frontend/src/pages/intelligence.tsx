...
export function NewsPageView() {
  ...
  const [scope, setScope] = useState<'ALL' | 'WATCHLIST' | 'HOLDINGS'>('ALL')
  ...
  const state = useRemote(() => scope === 'HOLDINGS' ? Promise.resolve({ items: [], nextCursor: null }) : api<NewsPage>(`/api/v1/news${queryString({ stockCode, sentiment, importance, marketImpact, marketImpactImportance, watchlist: scope === 'WATCHLIST' || null, from: from ? `${from}T00:00:00Z` : null, to: to ? `${to}T23:59:59Z` : null, sort, cursor, limit: 24 })}`), [stockCode, sentiment, importance, marketImpact, marketImpactImportance, scope, profile, from, to, sort, cursor])
...
export function NewsDetailPage({ articleId }: { articleId: string }) {
  ...
  const captureSelection = () => {
    ...
    setSelection({ text: selected, x: Math.min(Math.max(bounds?.left || 12, 12), window.innerWidth - 250), y: (bounds?.bottom || 80) + window.scrollY + 8 })
  }
  const explain = async () => {
    if (!selection) return
