...
export function NewsPageView() {
  ...
  const state = useRemote(() => api<NewsPage>(`/api/v1/news${queryString({ stockCode, sentiment, importance, marketImpact, from: from ? `${from}T00:00:00Z` : null, to: to ? `${to}T23:59:59Z` : null, sort, cursor, limit: 24 })}`), [stockCode, sentiment, importance, marketImpact, from, to, sort, cursor])
