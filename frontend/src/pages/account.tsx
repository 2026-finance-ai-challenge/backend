...
import type { Filing, InvestorType, NewsArticle, Profile, Stock } from '../types'
...
function AccountDashboard({ profile }: { profile: Profile }) {
  const watchlist = useRemote(() => api<{ items: Stock[] }>('/api/v1/market/stocks?watchlist=true&limit=75'), [profile])
  ...
  const feed = useRemote(async () => {
    const watched = await api<{ items: Stock[] }>('/api/v1/market/stocks?watchlist=true&limit=75')
    const pages = await Promise.all(watched.items.flatMap((stock) => [
      api<{ items: NewsArticle[] }>(`/api/v1/news?stockCode=${stock.stockCode}&limit=3`),
      api<{ items: Filing[] }>(`/api/v1/disclosures?stockCode=${stock.stockCode}&limit=3`),
    ]))
    return pages.flatMap((page, index) => index % 2 === 0
      ? (page.items as NewsArticle[]).map((news): FeedItem => ({ type: 'NEWS', date: news.publishedAt, news }))
      : (page.items as Filing[]).map((filing): FeedItem => ({ type: 'FILING', date: filing.detectedAt, filing })))
  ...
  const renameChat = async (room: Room) => { const name = window.prompt('New chat name', room.name)?.trim(); if (!name || name === room.name) return; try { await api(`/api/v1/me/chats/${room.id}/name`, { method: 'PUT', body: JSON.stringify({ name, expectedVersion: room.version }) }); chats.retry() } catch (reason) { setNotice(reason instanceof Error ? reason.message : 'Chat rename failed.') } }
  const deleteChat = async (room: Room) => { if (!window.confirm('Delete this chat? This chat will be removed from your chat history.')) return; try { await api(`/api/v1/me/chats/${room.id}`, { method: 'DELETE' }); chats.retry() } catch (reason) { setNotice(reason instanceof Error ? reason.message : 'Chat deletion failed.') } }
  const deleteAccount = async (event: FormEvent) => { event.preventDefault(); if (!window.confirm('Delete this account? Your private data will enter the published deletion process.')) return; setNotice(''); try { await api('/api/v1/me', { method: 'DELETE', body: JSON.stringify({ password: deletePassword }) }); session.clear(); navigate('home') } catch (reason) { setNotice(reason instanceof Error ? reason.message : 'Account deletion failed.') } }
  return <div className="content-page account-page"><PageTitle eyebrow="MY PAGE" title={`Welcome, ${profile.loginId}`} copy="Manage your personalized market research, AI chats and verified tax documents." />
    ...
    <div className="account-grid"><section><div className="split-heading"><h2>My Watchlist</h2><a href={appHash('screener')}>Find stocks →</a></div><RemoteBoundary state={watchlist}>{(value) => value.items.length ? <div className="compact-list">{value.items.map((item) => <a href={appHash('stock-detail', item.stockCode)} key={item.stockCode}><span className="stock-avatar">{item.nameEn.slice(0, 1)}</span><span><b>{item.nameEn}</b><small>{item.nameKo} · {item.stockCode} · {watchCautions(item)}</small></span><strong>{formatNumber(item.quote?.currentPriceKrw, { style: 'currency', currency: 'KRW', maximumFractionDigits: 0 })}<em className={(item.quote?.changeRate || 0) >= 0 ? 'up' : 'down'}>{item.quote?.changeRate === null || item.quote?.changeRate === undefined ? 'Unavailable' : `${item.quote.changeRate >= 0 ? '+' : ''}${item.quote.changeRate.toFixed(2)}%`}</em></strong></a>)}</div> : <StatusPanel title="No watchlist stocks" message="Use the heart button in search results or stock detail." />}</RemoteBoundary></section>
      <section><div className="split-heading"><h2>Recently Viewed</h2></div><RemoteBoundary state={recent}>{(items) => items.length ? <div className="compact-list">{items.map((item) => <a href={item.itemType === 'STOCK' ? appHash('stock-detail', item.referenceId) : item.itemType === 'FILING' ? appHash('filing-detail', item.referenceId) : appHash('news-detail', item.referenceId)} key={`${item.itemType}-${item.referenceId}`}><span className="recent-icon">{item.itemType.slice(0, 1)}</span><span><b>{item.itemType}</b><small>{item.referenceId}{item.stockCode && ` · ${item.stockCode}`}</small></span><time>{formatDate(item.viewedAt)}</time></a>)}</div> : <StatusPanel title="No recent research" message="Stocks, news and filings you open will appear here." />}</RemoteBoundary></section>
      <section className="wide"><div className="split-heading"><h2>Personalized Feed</h2><span>Watchlist news and filings</span></div><RemoteBoundary state={feed}>{(items) => items.length ? <div className="feed-list">{items.map((item) => item.type === 'NEWS' ? <a href={appHash('news-detail', item.news.id)} key={`N-${item.news.id}`}><span>NEWS</span><b>{item.news.englishTitle || item.news.originalTitle}</b><small>{item.news.relatedStocks.map((stock) => stock.stockCode).join(' · ')} · {formatDate(item.date)}</small></a> : <a href={appHash('filing-detail', item.filing.receiptNumber)} key={`F-${item.filing.receiptNumber}`}><span>FILING</span><b>{item.filing.titleEn || item.filing.titleKo}</b><small>{item.filing.stockCode} · {formatDate(item.date)}</small></a>)}</div> : <StatusPanel title="No personalized feed" message="Add a stock to your watchlist to collect related news and filings here." />}</RemoteBoundary></section>
      <section><div className="split-heading"><h2>My Chats</h2><span>Recent first</span></div><RemoteBoundary state={chats}>{(items) => items.length ? <div className="compact-list">{items.map((room) => <div key={room.id}><span className="recent-icon">AI</span><span><b>{room.name}</b><small>{room.context.type} · {room.context.title}</small></span><span className="row-actions"><button onClick={() => void renameChat(room)}>Rename</button><button onClick={() => void deleteChat(room)}>Delete</button></span></div>)}</div> : <StatusPanel title="No chat rooms" message="Open AI Agent from any screen to start a context-bound conversation." />}</RemoteBoundary></section>
    ...
    <section className="security-panel"><div><p className="eyebrow">SECURITY</p><h2>Session and password</h2><p>Access and refresh tokens are held in memory only. Server-side Redis state enables token rotation and immediate revocation.</p><button onClick={() => void api('/api/v1/auth/logout-all', { method: 'POST' }).finally(() => { session.clear(); navigate('home') })}>Log out all sessions</button><button onClick={() => void session.logout().then(() => navigate('home'))}>Log out this session</button></div><form onSubmit={changePassword}><label>Current password<input type="password" value={currentPassword} onChange={(event) => setCurrentPassword(event.target.value)} autoComplete="current-password" required /></label><label>New password<input type="password" value={newPassword} onChange={(event) => setNewPassword(event.target.value)} autoComplete="new-password" required /></label><button className="primary">Change password</button>{notice && <p className="form-hint" role="status">{notice}</p>}</form><form className="danger-zone" onSubmit={deleteAccount}><h3>Delete account</h3><p>Deletion revokes all Redis sessions immediately. Private data follows the published retention policy.</p><label>Confirm password<input type="password" value={deletePassword} onChange={(event) => setDeletePassword(event.target.value)} autoComplete="current-password" required /></label><button className="danger">Delete account</button></form></section>
  </div>
}

function watchCautions(stock: Stock): string {
  const warnings = [stock.quote?.viActive && 'VI', stock.quote?.singlePriceTrading && 'Single price', stock.quote?.tradingHalted && 'Halted', stock.quote?.priceLimitState && stock.quote.priceLimitState !== 'NONE' && stock.quote.priceLimitState].filter(Boolean)
  return warnings.length ? warnings.join(' · ') : stock.quote?.status || stock.market
}
