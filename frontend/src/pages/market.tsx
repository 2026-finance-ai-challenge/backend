...
export function ScreenerPage() {
  ...
  const profile = useProfile()
  const state = useRemote(() => api<{ count: number; items: Stock[] }>(`/api/v1/market/stocks${queryString({ market, sector, minChangeRate, maxChangeRate, minVolume, maxVolume, sort, tradingCaution: caution || null, watchlist: watchlist || null, limit: 75 })}`), [market, sector, minChangeRate, maxChangeRate, minVolume, maxVolume, sort, caution, watchlist, profile])
  return <div className="content-page"><PageTitle eyebrow="REAL-TIME STOCK SCREENER" title="Explore the supported market" copy="Filter 75 supported common stocks. Unavailable source data remains clearly unavailable." />
    <div className="filter-bar"><label>Market<select value={market} onChange={(event) => setMarket(event.target.value)}><option value="">All markets</option><option>KOSPI</option><option>KOSDAQ</option></select></label><label>Sector<input value={sector} maxLength={120} onChange={(event) => setSector(event.target.value)} placeholder="All sectors" /></label><label>Min change %<input type="number" step="0.01" value={minChangeRate} max={maxChangeRate || undefined} onChange={(event) => setMinChangeRate(event.target.value)} /></label><label>Max change %<input type="number" step="0.01" value={maxChangeRate} min={minChangeRate || undefined} onChange={(event) => setMaxChangeRate(event.target.value)} /></label><label>Min volume<input type="number" min="0" step="1" value={minVolume} max={maxVolume || undefined} onChange={(event) => setMinVolume(event.target.value)} /></label><label>Max volume<input type="number" min={minVolume || '0'} step="1" value={maxVolume} onChange={(event) => setMaxVolume(event.target.value)} /></label><label>Sort<select value={sort} onChange={(event) => setSort(event.target.value)}><option value="STOCK_CODE">Stock code</option><option value="CHANGE_DESC">Top gainers</option><option value="CHANGE_ASC">Top decliners</option><option value="VOLUME_DESC">Volume</option></select></label><label className="check"><input type="checkbox" checked={caution} onChange={(event) => setCaution(event.target.checked)} /> Trading caution</label><label className="check"><input type="checkbox" checked={watchlist} disabled={!profile} onChange={(event) => setWatchlist(event.target.checked)} /> My watchlist</label></div>
