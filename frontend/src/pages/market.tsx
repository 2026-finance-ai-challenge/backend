...
export function SearchPage({ query }: { query: string }) {
  ...
  const filings = useRemote(() => api<FilingPage>('/api/v1/disclosures?limit=20'), [query])
  const news = useRemote(() => api<NewsPage>('/api/v1/news?limit=20'), [query])
  const matchedFilings = useMemo(() => filings.data?.items.filter((item) => `${item.titleEn} ${item.titleKo} ${item.issuerNameEn} ${item.issuerNameKo} ${item.stockCode}`.toLowerCase().includes(query.toLowerCase())) || [], [filings.data, query])
  const matchedNews = useMemo(() => news.data?.items.filter((item) => `${item.englishTitle} ${item.originalTitle} ${item.what}`.toLowerCase().includes(query.toLowerCase())) || [], [news.data, query])
  return <div className="content-page"><PageTitle eyebrow="INTEGRATED SEARCH" title={`Search results for “${query}”`} copy="Stocks, filings and news are separated by source and relevance." /><div className="tabs" role="tablist">{(['all', 'stocks', 'filings', 'news'] as const).map((item) => <button role="tab" aria-selected={tab === item} className={tab === item ? 'active' : ''} onClick={() => setTab(item)} key={item}>{item[0].toUpperCase() + item.slice(1)}</button>)}</div>
    ...
    {(tab === 'all' || tab === 'filings') && <section className="result-section"><div className="split-heading"><h2>Filings</h2><span>{matchedFilings.length} matches in the current page</span></div>{filings.loading ? <div className="skeleton-stack"><i /><i /></div> : matchedFilings.length ? <div className="filing-list">{matchedFilings.map((filing) => <FilingRow filing={filing} key={filing.receiptNumber} />)}</div> : <StatusPanel title="No matching filing in the loaded page" message="Use DART Intelligence for full date and filing type filters." />}</section>}
    {(tab === 'all' || tab === 'news') && <section className="result-section"><div className="split-heading"><h2>News</h2><span>{matchedNews.length} matches in the current page</span></div>{news.loading ? <div className="skeleton-stack"><i /><i /></div> : matchedNews.length ? <div className="news-grid light">{matchedNews.map((article) => <NewsCard article={article} key={article.id} />)}</div> : <StatusPanel title="No matching news in the loaded page" message="Use News Intelligence for sentiment, importance and date filters." />}</section>}
  </div>
}
