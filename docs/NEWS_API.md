...
- 영문 본문과 EN/KR What/Why/Impact는 홈 호버 또는 상세 최초 진입 시에만 생성한다. 원문 해시·언어·정책 버전별로 저장해 모든 사용자가 재사용한다.
- 수집 트랜잭션은 OpenAI 응답을 기다리지 않고 원문·분류·영문 제목까지만 완료한다.
...
| `GET` | `/api/v1/news/{articleId}/translation?locale=en|ko` | 언어별 본문·What/Why/Impact 캐시 또는 생성 상태 조회 |
| `POST` | `/api/v1/news/{articleId}/translation?locale=en|ko` | 언어별 온디맨드 생성 또는 캐시 재사용 |
| `POST` | `/api/v1/news/{articleId}/term-explanations` | 기사에서 실제 선택한 텍스트의 금융 문맥 해설 |

목록 정렬은 `LATEST`, `IMPORTANCE`, `MARKET_IMPACT`를 지원한다. `MARKET_IMPACT`는 K-FNSPID 점수를 기준으로 정렬한다. 공개 API는 `FULL_ARTICLE`, 검증된 영문 제목, 분류 `READY`까지 준비된 대표 기사를 반환하며 본문·Insight 캐시 여부는 목록 노출을 막지 않는다.

주요 응답 필드는 `whatEn/whyEn/impactEn`, `whatKo/whyKo/impactKo`, `sentiment`, `importance`, `marketImpact`, confidence, 모델·프롬프트 버전이다.
...
| `FAILED` | 스키마·언어 검증 실패 후 자동 반복 호출 없이 종료됨 |

분류, 제목 번역, 본문·Insight 생성 상태는 독립적으로 관리한다. 온디맨드 생성은 PostgreSQL 유일 제약과 `FOR UPDATE SKIP LOCKED`, `sourceHash:locale` 분산 잠금으로 언어별 동시 요청을 하나의 작업으로 합친다.
...
뉴스 수집은 원문·분류·영문 제목까지만 완료한다. 본문 번역과 EN/KR What/Why/Impact는 최초 사용 시 생성하고 같은 캐시를 멱등 재사용한다.
