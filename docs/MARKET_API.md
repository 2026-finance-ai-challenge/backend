...
- 종목 검색의 `limit`은 최대 75개이며 전체 검색 결과 캐러셀은 이 범위를 사용한다.
...
KIS REST 현재가는 실시간 WebSocket과 구분해 `DELAYED`로 표시한다. VI·단일가 등 실시간 상태를 확인할 수 없는 응답은 `tradingStatusAvailable=false`로 내려가며 정상으로 추정하지 않는다. 외국인 한도 예측의 Min/Base/Max 단위는 한도 소진율이 아니라 외국인 보유율 `%`다. 장중·장외 모두 예측을 제공하고 `predictionSession`으로 `INTRADAY`(장중), `NEXT_SESSION`(다음 장)을 구분한다. 법정 한도 대상이 아닌 종목에는 예측을 제공하지 않는다.
...
날짜가 바뀌어도 개장 전에는 마지막 시세와 등락을 유지한다. 현재가·지수 REST 저장은 평일 09:00~18:00 KST로 제한하고 누락된 등락 필드는 0으로 저장하지 않는다. SSE는 같은 거래일의 최근 2분 표본만 정규장에 전달하며 전일 표본을 `LIVE`로 재생하지 않는다. 외국인 순매수는 개장 전 이전 평일을 기준으로 조회하고, 같은 날짜의 KOSPI·KOSDAQ 두 시장이 모두 있는 합계만 반환한다.
...
수집 직후와 평일 18:40에 AI 서비스의 승격된 종목별 모델로 Min/Base/Max를 재계산해 DB에 저장한다. `baseDate`는 입력 관측일이며, 모델이 추정하는 다음 거래일은 별도 `targetDate`로 반환한다. 정규장에는 당일, 장외에는 다음 개장 거래일(개장 전에는 당일)을 대상으로 저장된 예측을 우선 재사용한다. 전날 장외 예측은 다음 날 장중 예측으로 같은 수치를 사용하며 오래된 예측을 이후 거래일 값으로 재명명하지 않는다.

대상 거래일과 맞는 예측이 아직 없으면 `UNAVAILABLE`이며 이전 보유율은 유지한다. 원문 수집 전의 값을 임의로 만들어 채우지 않는다. 날짜 검증에서 만료된 예측은 `STALE`로 수치를 숨긴다. 거래일 계산은 Frontend와 동일한 2026년 KRX 주말·휴장일 목록을 사용한다. 다음 연도 달력은 [KRX 휴장일](https://open.krx.co.kr/contents/MKD/01/0110/01100305/MKD01100305.jsp) 공표 후 양쪽을 갱신해야 하며, 미등록 연도는 거래일을 추정하지 않는다.
