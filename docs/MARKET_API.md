...
취득 한도 집중 모니터링 4개 종목은 검증된 2019년 이후 KRX 학습 이력을 초기 적재하고, 이후 평일 장 마감 뒤 실제 보유수량을 멱등 누적한다. KRX 회원 수집기가 활성화된 환경은 최근 45일 이력을 보강하고, 그렇지 않은 환경은 KIS 현재가 응답의 외국인 보유수량을 최신 실제 거래일에 저장한다. 달력 날짜를 거래일로 추정하지 않는다.
