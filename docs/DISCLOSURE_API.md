...
- OpenAI에는 현재 질문과 검색된 한글 근거만 전달하고, 답변은 질문에서 로컬 판정한 EN/KR 언어로 생성한다.
...
현재 공시 RAG는 한글 원문 다국어 검색과 질문 언어의 EN/KR 답변을 사용한다. 사이트 언어·인용문·이전 대화는 새 답변 언어를 결정하지 않는다. 제목 번역 메모리는 목록·상세·영문 검색에서 `titleEn`을 재사용한다. 상세 최초 진입은 문서 단위 API 한 번으로 전체 섹션을 등록하고, 섹션 조회 API가 완료 상태를 갱신한다. 같은 원문 해시의 작업과 `READY` 결과는 모든 사용자에게 재사용한다.
