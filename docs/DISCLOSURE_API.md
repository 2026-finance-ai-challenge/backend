...
현재 공시 RAG는 한글 원문 다국어 검색과 영어 답변을 사용한다. 제목 번역 메모리와 기존 234,071건 백필은 완료됐으며 목록·상세·영문 검색에서 `titleEn`을 재사용한다. 섹션 온디맨드 번역 API는 다음 구현 단위로 [구현 현황](IMPLEMENTATION_STATUS.md)에서 `개선 예정`으로 관리한다.
