...
| 뉴스 인텔리전스 | 홈 카드 hover, 피드/상세, 필터, 본문 선택 해설 | Naver 수집, 중복 군집, Hana 금융 분류, K-FNSPID 영향 점수, OpenAI 번역·What/Why/Impact | 분류 모델 commit·SHA 검증과 실제 추론, OpenAI 스키마·실패 경로 테스트 |
| DART Intelligence | 목록/상세, 목차·표·문단, 버전·원문, AI 요약·질문 | ZIP·SHA-256 카탈로그, 구조화 payload, 공시별 HALFVEC 검색, 근거 고정 답변 | 접수번호·문서 버전 격리, 인용 검증, 근거 부족 거절, 색인 상태 구현 |
