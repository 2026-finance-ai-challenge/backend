...
| 뉴스 인텔리전스 | 영어 제목 목록, 상세 최초 번역·AI Insight, 캐시 재조회, 본문 선택 해설 | Naver 수집, 제목 번역 메모리, 온디맨드 본문·What/Why/Impact, Hana 금융 분류, K-FNSPID 영향 점수 | 구현 예정: 수집 시 본문 미생성, 동시 최초 요청 1회 생성, 캐시 적중·실패·전문 미제공 상태 검증 |
| DART Intelligence | 영어 제목 목록, 목차·표·문단, 섹션 온디맨드 번역, 버전·원문, AI 요약·질문 | ZIP·SHA-256 카탈로그, 제목 번역 메모리, 구조화 payload, 공시별 HALFVEC 한글 검색·영어 답변 | RAG 격리·인용·거절은 구현 완료. 제목·섹션 번역 캐시와 고유 제목 백필은 구현 예정 |
