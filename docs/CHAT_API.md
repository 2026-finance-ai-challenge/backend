...
- 일반·종목 채팅의 공시 목록 질문은 제목·공시일·접수번호 메타데이터를 조회한다. 실적·공시 내용 질문은 질문에서 식별한 지원 종목과 기간을 제한한 뒤 내부 공시 evidence API로 현재 문서 원문과 벡터 청크를 검색한다. `FILING` 문맥은 연결된 한 공시로 범위를 더 좁힌다.
- 인용 ID는 내부 AI 계약인 `E1` 형식으로 부여하며 실제 뉴스 UUID·DART 접수번호·출처 링크는 서버에서 대응시킨다.
- 이름 식별은 종목코드, 정식 한·영문명, DB 별칭, 제한된 영문 오타 순서로 수행한다. 대명사와 임의의 자연어 날짜 범위는 연결된 종목 문맥 또는 명시적 연월이 없으면 지원을 보장하지 않는다.
